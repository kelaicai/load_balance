...
int main()
{
   ...
   listen(sockfd,5); 
    ...
    bzero(&my_addr,0);
	res=getsockname(sockfd,(struct sockaddr*)&my_addr,&len);
    if(res<0)
    {
        perror("getsockname error");
    }
    printf("my addr port is %d  len=%d\n",ntohs(my_addr.sin_port),len);
	...
	while(true)
	{
		int c=accept(sockfd,(struct sockaddr*)&caddr,&len2);
		if(c<0)
		{
			continue;
		}
